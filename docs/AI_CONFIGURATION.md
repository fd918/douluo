...
项目同时支持两种 AI 路径。GitHub Pages 短地址使用“玩家自带 AI”：玩家在游戏内填写自己的地址、模型和密钥，连接成功后浏览器直接请求服务商。Sites 版本继续调用自己的 `/api/ai/generate`，由服务端访问 AI 供应商；公网服务商、价格、主备顺序和限流策略由本机可视化中台管理并保存到 Sites D1。两种路径失败时都会自动使用本地剧情。

## GitHub Pages 玩家自带 AI

手机打开 `https://fd918.github.io/douluo/`，进入“档案 → 玩家自带 AI”。页面默认填写：

- API 地址：`https://apihub.agnes-ai.com/v1`
- 模型 ID：`agnes-2.5-flash`
- API 密钥：空，需要玩家填写自己的完整密钥

点击“保存并测试连接”。成功后状态变为“AI 剧情已启用”；401、超时、跨域、模型错误或返回内容无效时，会显示原因并保持本地剧情模式。以后更换服务商只需整体替换这三个输入框，不需要修改代码或重新部署。

玩家配置保存在当前浏览器的 `douluo.browser-ai.v1` 本地存储中，不会上传到 GitHub 或运营中台。公共电脑使用后应点击“关闭 AI 并清除密钥”。此模式的请求不会经过 Sites Worker，因此不使用 D1 限流、主备路由、调用统计或费用看板；实际额度和费用由玩家所填服务商账号承担。服务商还必须允许网页跨域访问 `/v1/chat/completions`。
...
上线后不上传 `.env.local`。GitHub Pages 静态前端 `https://fd918.github.io/douluo/` 使用玩家自带 AI，不包含项目方真实密钥；Sites 地址继续使用 Worker 托管 AI，真实密钥、模型和限流只存在 Sites 生产环境变量。
...
更换 Sites 托管版本的服务商时，也只在部署平台或运营中台修改配置，不修改前端剧情代码。GitHub Pages 只能托管静态文件，项目方不能在其中安全保存统一密钥；当前 Pages 版本因此只接受玩家自己输入并承担的密钥。正式大规模运营仍推荐 Sites Worker 托管模式。
