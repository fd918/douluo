...
- New players enter a skippable narrated world prologue before character creation. Fixed prologue, six martial-soul openings and main-story choices use packaged MP3 narration so Xiaomi/WeChat WebViews do not depend on `speechSynthesis`; dynamic AI/free-text narration may use the device's Chinese system voice when available. Narration defaults to automatic playback, persists independently under `douluo.narration.settings.v1`, and provides 44px toggle, pause/resume and replay controls. Duck BGM while narration speaks.
- Story-choice results must bridge into the next chapter, time, location and objective. Travel and random world events may add timeline records but must not overwrite the current main-story node or main-story narration.
- Inventory rows use item-specific game artwork from `public/game-assets/items/`; recommendation letters, herbs, equipment, crystals, potions and soul bones must remain visually recognizable rather than reverting to category-only glyphs. Item detail sheets use the established dark navy, antique-gold, cyan and violet theme rather than the runtime's default white surface.
